...
void TicketManager2::showGeneralTicket(const bool& card)
{
	...
	cout << "----------------------------" << endl;
	for (size_t i = 0; i < count; i++) {
		GeneralTicket* t = dynamic_cast<GeneralTicket*>(ticket[i].get()); //ticket[i] : Ticket* -> GeneralTicket*로 다운캐스팅
		if (t != nullptr) {
...
void TicketManager2::showAdvancedTicket()
{
	...
	for (size_t i = 0; i < count; i++) {
		AdvancedTicket* t = dynamic_cast<AdvancedTicket*>(ticket[i].get()); //ticket[i] : Ticket* -> AdvancedTicket*로 다운캐스팅
		if (t != nullptr) {
